...
OUT_DIR = Path(f"./total_score4__all_results_on_higher_temp_T_0.5_no_interface/design_results_{f_name}_contact_loss_{MAX_ITERATIONS}_higher_temp_T_0.5")
...
def get_chain_ids_from_yaml(path,ccd_path):
    with ccd_path.open("rb") as file:
        ccd = pickle.load(file)  # noqa: S301
...
def get_seq_proteinmpnn_for_renew(output_dir,pdb_path):
    output_dir = Path(output_dir)
    #cur_dir = Path(cur_dir)
    # Assume f_name is derived from pdb_path
    f_name = Path(pdb_path).stem
    ...
    for iteration in range(start_iter,MAX_ITERATIONS):
        ...
        if iteration == start_iter:
            ...
            try:
                ...
                pdb_path2 = f"{f_name}_model_0.pdb"
                new_sequence = get_seq_proteinmpnn_for_renew(protmpnn_results_dir,pdb_path2)
                print("read proteinmpnn file sucessfully, the new_seq is ", new_sequence)
            except:
                print("attention! continue from last checkpoint seq not working")
        ...
        if iteration==start_iter:
            target_id_name = f_name
            all_chain_ids = [c.chain_name for c in records.record.chains]
            print("all chain_ids resume here",all_chain_ids)
