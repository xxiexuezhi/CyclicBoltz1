...
from boltz.model.model_cc import Boltz1
...
OUT_DIR = Path(f"./sidechain_cc_generation_with_total_score4__all_results_on_higher_temp_T_0.5_no_interface/design_results_{f_name}_contact_loss_{MAX_ITERATIONS}_higher_temp_T_0.5")
...
def main(start_step=0,start_iter=0): ## given the nums if continues. else default 0
    ...
    for iteration in range(start_iter,MAX_ITERATIONS):
        ...
        if iteration == start_iter:
            ...
            # force c positons for sidechain bridge
            new_sequence_lst = list(new_sequence)
            new_sequence_lst[1] = "C"
            new_sequence_lst[-2] = "C"
